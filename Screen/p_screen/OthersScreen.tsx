...
const OthersScreen = () => {
  const [otherItems, setOtherItems] = useState<Figurine[]>([]);
  ...
  const _queryOtherItem = () =>{
    try{
      const FigmaData:any = [];
      db.executeSql('SELECT * FROM figurines WHERE tab_category="Other Figure"',[], (results:any) => {
      (results.rows.raw()).forEach(( item:any ) => {
      FigmaData.push(item);
      })
      setOtherItems(FigmaData);
      });
      } catch (error) {
      console.error(error);
  ...
  const numberOfTabs = Math.ceil(otherItems.length / pageSize);
  ...
  const getItemsForTab = (tab: number) => {
    ...
    return otherItems.slice(startIndex, endIndex);
  };
