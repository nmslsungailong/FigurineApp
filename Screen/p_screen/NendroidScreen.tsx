...
const NendroidScreen = () => {
  const [nendroidItems, setNendroidItems] = useState<Figurine[]>([]);
  ...
  const _queryNendroidItem = () =>{
    try{
      const FigmaData:any = [];
        db.executeSql('SELECT * FROM figurines WHERE tab_category="Nendroid"',[], (results:any) => {
      (results.rows.raw()).forEach(( item:any ) => {
        FigmaData.push(item);
      })
        setNendroidItems(FigmaData);
      });
      } catch (error) {
        console.error(error);
  ...
  const numberOfTabs = Math.ceil(nendroidItems.length / pageSize);
  ...
  const getItemsForTab = (tab: number) => {
    ...
    return nendroidItems.slice(startIndex, endIndex);
  };
