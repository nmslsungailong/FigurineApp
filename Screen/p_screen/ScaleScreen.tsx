...
const ScaleScreen = () => {
  const [scaleItems, setScaleItems] = useState<Figurine[]>([]);
  ...
  const _queryScaleItem = () =>{
    try{
      const FigmaData:any = [];
      db.executeSql('SELECT * FROM figurines WHERE tab_category="Scale Figure"',[], (results:any) => {
      (results.rows.raw()).forEach(( item:any ) => {
      FigmaData.push(item);
      })
      setScaleItems(FigmaData);
      });
      } catch (error) {
      console.error(error);
  ...
  const numberOfTabs = Math.ceil(scaleItems.length / pageSize);
  ...
  const getItemsForTab = (tab: number) => {
    ...
    return scaleItems.slice(startIndex, endIndex);
  };
